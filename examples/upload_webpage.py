...
if __name__=='__main__':
    # Create Safe opject
    s = Safe('Website Upload Test', '0.0.1', 'hintofbasil', 'com.github.hintofbasil')
    # Authenticate against safe network
    if s.authenticate(permissions=['SAFE_DRIVE_ACCESS']):
        # Check if folder already exists
        valid, _ = s.get_dir(folder, isPathShared=True)
        # If it doesn't exist create folder
        if not valid:
            s.mkdir(folder, False, False, True)
        ...
        s.put_file(fileData, folder + filename, isPathShared=True)
        # TODO get_dns
        # Create DNS entry
        s.post_dns(dns_name, 'www', folder, isPathShared=True)
